...
func TestReconcile(t *testing.T) {
	...
	t.Run("controller should not deactivate user", func(t *testing.T) {
		...
		t.Run("nstemplatetier", func(t *testing.T) {
			...
			t.Run("usersignup should not be deactivated - basic tier (30 days)", func(t *testing.T) {
				// given
				murProvisionedTime := metav1.Now()
				mur := murtest.NewMasterUserRecord(t, username, murtest.TierName(basicTier.Name), murtest.Account("cluster1"), murtest.ProvisionedMur(&murProvisionedTime), murtest.UserIDFromUserSignup(userSignupFoobar))
				mur.Labels[toolchainv1alpha1.MasterUserRecordOwnerLabelKey] = userSignupFoobar.Name
			...
			t.Run("user deactivation excluded", func(t *testing.T) {
				...
				mur := murtest.NewMasterUserRecord(t, username, murtest.TierName(basicTier.Name), murtest.Account("cluster1"), murtest.ProvisionedMur(murProvisionedTime), murtest.UserIDFromUserSignup(userSignupRedhat))
				mur.Labels[toolchainv1alpha1.MasterUserRecordOwnerLabelKey] = userSignupRedhat.Name
	...
	t.Run("controller should deactivate user", func(t *testing.T) {
		...
		t.Run("nstemplatetier", func(t *testing.T) {
			...
			t.Run("usersignup should be marked as deactivating - basic tier (30 days)", func(t *testing.T) {
				...
				mur := murtest.NewMasterUserRecord(t, username, murtest.TierName(basicTier.Name), murtest.Account("cluster1"),
					murtest.ProvisionedMur(murProvisionedTime), murtest.UserIDFromUserSignup(userSignupFoobar))
				mur.Labels[toolchainv1alpha1.MasterUserRecordOwnerLabelKey] = userSignupFoobar.Name
	...
	t.Run("failures", func(t *testing.T) {
		...
		t.Run("unable to get UserTier", func(t *testing.T) {
			...
			// then
			require.Error(t, err)
			require.Contains(t, err.Error(), `nstemplatetiers.toolchain.dev.openshift.com "deactivate30" not found`)
			assertThatUserSignupDeactivated(t, cl, username, false)
		})
