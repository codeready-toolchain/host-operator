...
func TestCreateSpaceBindingRequest(t *testing.T) {
	...
	sbr := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
		spacebindingrequesttestcommon.WithMUR("jane"),
	...
	t.Run("success", func(t *testing.T) {
		...
		t.Run("spaceBinding doesn't exists it should be created", func(t *testing.T) {
			...
			spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, sbr.GetNamespace(), sbr.GetName(), member1.Client).
		...
		t.Run("member1 GET request fails, member2 GET returns not found but SpaceBindingRequest is on member3", func(t *testing.T) {
			...
			require.NoError(t, err)
			// spaceBindingRequest exists with config and finalizer
			spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, sbr.GetNamespace(), sbr.GetName(), member3.Client).
				HasSpecSpaceRole("admin").
				HasSpecMasterUserRecord(janeMur.Name).
				HasConditions(spacebindingrequesttestcommon.Ready()).
				HasFinalizer()
	...
	t.Run("failure", func(t *testing.T) {
		...
		t.Run("MasterUserRecord cannot be blank", func(t *testing.T) {
			// given
			badSBR := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
				spacebindingrequesttestcommon.WithMUR(""), // empty MUR
				spacebindingrequesttestcommon.WithSpaceRole("admin"))
			...
			// spaceBindingRequest exists with config and finalizer
			spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, badSBR.GetNamespace(), badSBR.GetName(), member1.Client).
				HasSpecSpaceRole("admin").
		...
		t.Run("SpaceRole cannot be blank", func(t *testing.T) {
			// given
			badSBR := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
				spacebindingrequesttestcommon.WithMUR(janeMur.Name),
				spacebindingrequesttestcommon.WithSpaceRole("")) // empty spacerole
			member1 := NewMemberClusterWithClient(test.NewFakeClient(t, badSBR, sbrNamespace), "member-1", corev1.ConditionTrue)
			hostClient := test.NewFakeClient(t, janeSpace, janeMur, base1nsTier)
			ctrl := newReconciler(t, hostClient, member1)
		...
		t.Run("unable to find space label in spaceBindingRequest namespace", func(t *testing.T) {
			...
			sbr := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", sbrNamespace.GetName())
			member1Client := test.NewFakeClient(t, sbr, sbrNamespace)
			member1 := NewMemberClusterWithClient(member1Client, "member-1", corev1.ConditionTrue)
			hostClient := test.NewFakeClient(t, base1nsTier)
			ctrl := newReconciler(t, hostClient, member1)
		...
		t.Run("invalid SpaceRole", func(t *testing.T) {
			// given
			invalidSBR := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", sbrNamespace.GetName(), spacebindingrequesttestcommon.WithSpaceRole("maintainer"), spacebindingrequesttestcommon.WithMUR(janeMur.Name)) // invalid role
			...
			require.EqualError(t, err, cause)
			spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, invalidSBR.GetNamespace(), invalidSBR.GetName(), member1.Client).
				HasConditions(spacebindingrequesttestcommon.UnableToCreateSpaceBinding(cause)).
				HasFinalizer()
		})
		...
		t.Run("There are more than one SpaceBinding for the given MUR and Space", func(t *testing.T) {
			...
			sbrForDuplicatedSpaceBinding := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
				spacebindingrequesttestcommon.WithMUR(janeMur.Name),
				spacebindingrequesttestcommon.WithSpaceRole("admin"))
		...
		t.Run("SpaceBindingRequest CR is deleted but admin-generated SpaceBinding stays there", func(t *testing.T) {
			...
			sbrForDuplicatedSpaceBinding := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
				spacebindingrequesttestcommon.WithMUR(janeMur.Name),
				spacebindingrequesttestcommon.WithSpaceRole("admin"),
				spacebindingrequesttestcommon.WithFinalizer(), // we set the finalizer so we can check if it's being removed
				spacebindingrequesttestcommon.WithDeletionTimestamp(),
			)
...
func TestDeleteSpaceBindingRequest(t *testing.T) {
	...
	sbr := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", "jane-tenant",
		spacebindingrequesttestcommon.WithMUR("jane"),
		spacebindingrequesttestcommon.WithSpaceRole("admin"),
		spacebindingrequesttestcommon.WithDeletionTimestamp(), // spaceBindingRequest was deleted
		spacebindingrequesttestcommon.WithFinalizer())         // has finalizer still
	t.Run("success", func(t *testing.T) {
		t.Run("spaceBindingRequest should be in terminating while spacebinding is deleted", func(t *testing.T) {
			...
			t.Run("spaceBindingRequest is deleted when spacebinding is gone", func(t *testing.T) {
				...
				require.NoError(t, err)
				spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, sbr.GetNamespace(), sbr.GetName(), member1.Client).DoesNotExist() // spaceBindingRequest is gone
			})
		})
		...
		t.Run("finalizer was already removed", func(t *testing.T) {
			...
			sbrNoFinalizer := spacebindingrequesttestcommon.NewSpaceBindingRequest("lana", "lana-tenant",
				spacebindingrequesttestcommon.WithMUR("lana"),
				spacebindingrequesttestcommon.WithSpaceRole("admin"),
				spacebindingrequesttestcommon.WithDeletionTimestamp()) // spaceBindingRequest was deleted
			...
			// then
			require.NoError(t, err)
			spacebindingrequesttestcommon.AssertThatSpaceBindingRequest(t, sbrNoFinalizer.GetNamespace(), sbrNoFinalizer.GetName(), member1.Client).HasNoFinalizers() // spaceBindingRequest is gone
		})
	})
	...
	t.Run("failure", func(t *testing.T) {
		t.Run("SpaceBinding resource is already being deleted for more than 2 minutes", func(t *testing.T) {
			...
			sbr := spacebindingrequesttestcommon.NewSpaceBindingRequest("jane", sbrNamespace.GetName(),
				spacebindingrequesttestcommon.WithDeletionTimestamp(),
				spacebindingrequesttestcommon.WithFinalizer(),
				spacebindingrequesttestcommon.WithMUR(janeMur.Name),
