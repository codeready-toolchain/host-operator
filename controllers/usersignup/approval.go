...
func getClusterIfApproved(cl client.Client, userSignup *toolchainv1alpha1.UserSignup, getMemberClusters cluster.GetMemberClustersFunc) (bool, targetCluster, error) {
	...
	if err := cl.Get(context.TODO(), types.NamespacedName{Namespace: userSignup.Namespace, Name: commontoolchaincfg.ToolchainStatusName}, status); err != nil {
...
func hasNotReachedMaxNumberOfUsersThreshold(config commontoolchaincfg.ToolchainConfig, counts counter.Counts) cluster.Condition {
...
func hasEnoughResources(config commontoolchaincfg.ToolchainConfig, status *toolchainv1alpha1.ToolchainStatus) cluster.Condition {
	return func(cluster *cluster.CachedToolchainCluster) bool {
