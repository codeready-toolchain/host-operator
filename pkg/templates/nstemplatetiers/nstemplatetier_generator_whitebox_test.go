			...
			for _, actual := range tierObjs {
				...
				for kind, tmpl := range templatesByTier[tier].namespaceTemplates {
					...
					for _, ns := range actual.Spec.Namespaces {
						...
						// LimitRange
						containsObj(t, ns.Template, fmt.Sprintf(`{"apiVersion":"v1","kind":"LimitRange","metadata":{"labels":{"toolchain.dev.openshift.com/provider":"codeready-toolchain"},"name":"resource-limits","namespace":"${USERNAME}-%s"},"spec":{"limits":[{"default":{"cpu":"500m","memory":"2Gi"},"defaultRequest":{"cpu":"100m","memory":"64Mi"},"type":"Container"}]}}`, ns.Type))
						// NetworkPolicies
