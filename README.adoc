...
If there is any broken release that cannot be built & pushed through the pipeline and has to be done/fixed manually then do the following steps:
...
2. Checkout to the problematic commit that failed in the pipeline and that has to be manually released.
